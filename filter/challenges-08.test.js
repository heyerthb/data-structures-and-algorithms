...
const oddValues = (arr) => arr.filter( element => (element % 2) );
...
const filterStringsWithVowels = (arr) => arr.filter(element => !(/^[aeiou]+$/));
...
const notInFirstArray = (forbiddenValues, arr) => forbiddenValues.filter(element => (arr));
// const notInFirstArray = (forbiddenValues, arr) => arr.filter(element => (forbiddenValues);
...
arr.filter(element=>(minBaseStat < 50))
};
// {
//   let newArr = []
// arr.filter(element => (minBaseStat));
// newArr.push(arr)
// };
